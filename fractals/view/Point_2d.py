...
class Point_3d(Point_2d):
    def __init__(self, x, y, z=0, color=Settings.COLOR_LINE, fontText=('Arial', 8, 'bold'), colorText='black', showComments=False):
        super(Point_3d, self).__init__(x, y, color, fontText, colorText, showComments)

        self.z = z

    # Функция берет реальные координаты точки и возвращает координаты для экрана (в 2d уже)
    def coordShift(self, field):
        try:
            x, y = field.coordinateShift_2d(self)
        except:
