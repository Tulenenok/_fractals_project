...
class CanvasSegment(Line):
    ...
    def show(self, field):
        ...
        if self.dash and self.needDash:
            self.l = field.create_line(xS, yS, xE, yE, fill=self.color, width=self.width, dash=self.dash, tag='line')
        else:
            self.l = field.create_line(xS, yS, xE, yE, fill=self.color, width=self.width, tag='line')
